...
app.post('/login', (req, res) => {
  ...
  const connectionObj = new pgClient.Pool({user:'uapv2102872', host:'127.0.0.1', database:'etd', password: 'jhFP6M', port:5432});
  connectionObj.connect((err, client, done) => {
    if(err) {
      console.log('Erreur de connexion au serveur pg' + err.stack);
    } else {
      console.log('Connection établie / pg db server');
      ...
      client.query(sql, (err, result) => {
        if(err) {
          ...
          const updateSql = `UPDATE fredouil.users SET statut_connexion = 1 WHERE identifiant = $1;`;
          const values = [identifiant];

          client.query(updateSql, values, (err, updateResult) => {
            if (err) {
